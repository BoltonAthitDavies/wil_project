...
"""Lightweight 2D top-down viewer for the warehouse sim: watch and drive, no 3D.
    ...
    python3 ~/wil_project/script/viewer.py
...
LOCALIZATION MODE (no ground truth)
    When rtabmap is localizing against a pre-built map there is no ground truth to
    align to, and the viewer's job changes: it stops being an error meter and
    becomes a localization-state indicator.

        python3 ~/wil_project/script/viewer.py --gt-topic '' --align none --map ros

    --gt-topic '' turns ground truth off. That matters beyond the trail: on_gt is
    normally the ONLY writer of sim_t and rtf, so with it gone the front-end
    odometry becomes the clock instead (see on_est) and the RTF headline keeps
    working. --align none draws each estimator raw; under the localization frame
    scheme the front-end publishes into `odom`, so the raw trail is already in map
    coordinates up to the map->odom correction, and the gap between it and the
    `loc` row IS that correction.

    The `loc` row reads /localization_pose. rtabmap writes a 9999 covariance
    diagonal on every update where it did not relocalize, so the row is red
    SEARCHING until the first fix and green with a real sd afterwards. This viewer
    stays READ-ONLY about localization -- it consumes no TF and sends no pose
    estimates. Use RViz to drive the modes; use this to watch cheaply.
...
# Two roots for model:// URIs, searched in order, the way gz searches its resource
# path -- see bake_map.resolve_uri(). The share dir itself is needed because
# small_warehouse_static_tinymap.world names its models `model://model_tiny/<model>`
# to pull the pristine SMALL warehouse out of model_tiny/ rather than the rescaled
# large one in models/; the models/ entry resolves every other world's plain
# one-segment URIs.
MODELS = [PKG, os.path.join(PKG, 'models')]
...
# rtabmap writes a 9999 diagonal into /localization_pose on every update where it
# has NOT relocalized (CoreWrapper.cpp:2419-2426). That is the machine-readable
# "I do not know where I am", and it is why pub_loc_pose_only_when_localizing must
# be left false -- with it true the topic goes silent instead, which is the same
# thing as a crashed node from the viewer's side.
LOC_NOT_LOCALIZED = 9000.0
...
class Estimator(object):
    ...
    def __init__(self, key, label, topic, align_mode, trail_min_step):
        ...
        # (cos, sin, tx, ty), estimator -> world. align_mode 'none' means draw the
        # raw estimate, so seed the IDENTITY rather than None: on_est only appends
        # to the trail when align is not None, so leaving it None made --align none
        # a silent no-op that drew nothing at all. Identity is also the RIGHT answer
        # under the localization frame scheme, where the front-end publishes into
        # `odom` and the raw trail is already in map coordinates up to the map->odom
        # correction -- drawing it beside /localization_pose shows that correction.
        self.align = self._seed_align()

    def _seed_align(self):
        return (1.0, 0.0, 0.0, 0.0) if self.align_mode == 'none' else None
...
class SharedState(object):
    ...
    def __init__(self, args):
        ...
        self.teleop = 'off'
        # rtabmap localization, read-only. loc is the latest sample whatever its
        # covariance; loc_ok_t is the stamp of the last one that was actually
        # localized, so the panel can say how long ago the fix was lost.
        self.loc = None                   # (x, y, yaw, cov_xx, stamp)
        self.loc_ok_t = None
...
class RosLink(object):
    ...
    def __init__(self, args, state):
        ...
        self.expected = {}
        # Guarded: --gt-topic '' is the no-ground-truth mode, and an empty topic
        # name raises InvalidTopicNameException here and kills the viewer.
        if args.gt_topic:
        ...
        if args.loc_topic:
            # rate=True with no expected_hz: show the arrival rate, pass no judgement
            # on it (the /clock idiom). It should equal Rtabmap/DetectionRate, so the
            # row is diagnostic on its own.
            self._sub(PoseWithCovarianceStamped, args.loc_topic, self.on_loc,
                      LATEST, None, rate=True)
    ...
    def on_est(self, msg, est):
        ...
        with self.state.lock:
            st = self.state
            # With no ground truth this is the only sim-stamped stream left, so it
            # becomes the clock. Same argument __init__ makes for using odometry
            # header stamps rather than wall time.
            if not self.args.gt_topic:
                st.sim_t = t
                st.rtf.add(t)
            est.raw = (p.x, p.y, yaw)
    ...
    def on_loc(self, msg):
        ...
        if not (math.isfinite(p.x) and math.isfinite(p.y)):
            return
        t = msg.header.stamp.sec + msg.header.stamp.nanosec * 1e-9
        cov = msg.pose.covariance[0]      # xx; rtabmap sets the whole diagonal
        with self.state.lock:
            self.state.loc = (p.x, p.y, yaw_of(msg.pose.pose.orientation), cov, t)
            if cov < LOC_NOT_LOCALIZED:
                self.state.loc_ok_t = t
        self._rate(self.args.loc_topic)
...
def _dae_diffuse(dae_path):
    """{material id: {'image': abs path or None, 'color': (r,g,b) 0-1 or None}}.

    Reads what each material's <diffuse> actually is, rather than assuming a
    texture. The plain ground model has no images at all -- its concrete diffuse is
    a flat <color> -- so 'is there a texture' has to be a question, not a given.
    """
    import xml.etree.ElementTree as ET
    ns = '{http://www.collada.org/2005/11/COLLADASchema}'
    root = ET.parse(dae_path).getroot()
    base = os.path.dirname(dae_path)

    images = {}
    for im in root.iter(ns + 'image'):
        f = im.find(ns + 'init_from')
        if f is not None and f.text:
            images[im.get('id')] = os.path.normpath(os.path.join(base, f.text.strip()))

    fx = {}
    for eff in root.iter(ns + 'effect'):
        dif = eff.find('.//%sdiffuse' % ns)
        img = col = None
        if dif is not None:
            tex = dif.find(ns + 'texture')
            if tex is not None:
                img = images.get(tex.get('texture'))
            c = dif.find(ns + 'color')
            if c is not None and c.text:
                v = [float(x) for x in c.text.split()]
                if len(v) >= 3:
                    col = tuple(v[:3])
        fx[eff.get('id')] = {'image': img, 'color': col}

    out = {}
    for mat in root.iter(ns + 'material'):
        ie = mat.find(ns + 'instance_effect')
        url = (ie.get('url') if ie is not None else '') or ''
        out[mat.get('id')] = fx.get(url.lstrip('#'), {'image': None, 'color': None})
    return out


def ground_assets(world_path, models=MODELS):
    """What the floor of THIS world is made of: (dae, concrete, atlas, n_marks).

    The viewer used to hard-code the canonical GroundB_01 mesh and its two
    textures. That is wrong for any world including a different ground model --
    the *_nofloortexture worlds include GroundB_01_plain -- and it failed the bad
    way: the floor still rendered, complete with 154 painted bay outlines that are
    not in the world at all, so the overlay disagreed with what the cameras see.

    `concrete` is a texture path when the model has one, else an (r,g,b) 0-1 tuple
    read from its flat diffuse. `atlas` is None when the model has no markings.
    """
    model = next((m for m, _n, _p in bake_map.load_world(world_path)
                  if 'GroundB' in m), None)
    if model is None:
        return None
    base = bake_map.resolve_uri(model, models)
    leaf = os.path.basename(model)
    dae = os.path.join(base, 'meshes', leaf + '_visual.DAE')
    if not os.path.isfile(dae):
        return None

    diffuse = _dae_diffuse(dae)
    n_marks = len(_marking_faces(dae, MARK_MATERIAL))
    mark = diffuse.get(MARK_MATERIAL, {})
    concrete = next((v for k, v in diffuse.items() if k != MARK_MATERIAL),
                    {'image': None, 'color': None})
    return (dae,
            concrete['image'] or concrete['color'] or (0.5, 0.5, 0.5),
            mark.get('image') if n_marks else None,
            n_marks)
...
def build_floor_array(path, ppm, flip):
    ...
    """
    w0 = int(round((FLOOR_X1 - FLOOR_X0) * ppm))
    h0 = int(round((FLOOR_Y1 - FLOOR_Y0) * ppm))
    if not isinstance(path, str):
        # A flat diffuse colour instead of a texture: the plain ground model has
        # no image to tile, so there is nothing to fit UVs against.
        rgb = np.array([int(round(c * 255)) for c in path[:3]], dtype=np.uint8)
        return np.ascontiguousarray(np.broadcast_to(rgb, (h0, w0, 3)).copy())
...
class MapView(QWidget):
    ...
    def __init__(self, args, state, ros):
        ...
        # (x, y, yaw_rad) placing the map overlay in world coords, or None.
        self.map_pose = None
        if getattr(args, 'map_pose', ''):
            try:
                mx, my, myaw = (float(v) for v in args.map_pose.split(','))
                self.map_pose = (mx, my, math.radians(myaw))
            except ValueError:
                state.warn('--map-pose wants X,Y,YAW_DEG; ignoring %r'
                           % args.map_pose)
        # What this world's floor is actually made of. Only overrides --texture
        # when the user left it at the default, so an explicit --texture still wins.
        ground_dae = atlas = None
        texture = args.texture
        n_marks = None
        try:
            g = ground_assets(args.world)
            if g is not None:
                ground_dae, concrete, atlas, n_marks = g
                if args.texture == TEXTURE:
                    texture = concrete
        except Exception as e:
            state.warn('could not resolve ground model (%s); assuming stock floor' % e)

        if not args.no_floor:
            try:
                self.floor_pm = build_floor_pixmap(texture, args.floor_ppm,
                                                   args.floor_flip)
            except Exception as e:
                state.warn('floor texture unavailable (%s); flat fill' % e)
            if n_marks == 0:
                # Not a failure: this world's ground model has no painted lines.
                print('[viewer] %s has no floor markings'
                      % os.path.basename(args.world))
                self.show_marks = False
            if self.floor_pm is not None and not args.no_markings and n_marks != 0:
                ...
                try:
                    img = build_floor_array(texture, args.floor_ppm, args.floor_flip)
                    n = paint_markings(img, args.floor_ppm, args.floor_flip,
                                       dae_path=ground_dae, atlas_path=atlas)
                    h, w = img.shape[:2]
                    img = np.ascontiguousarray(img)
    ...
    def _draw_grid_overlay(self, p):
        ...
        # A SLAM database is built in the front-end's frame, anchored wherever it
        # initialised -- not in Gazebo world coordinates. So its cells land offset
        # and rotated from the real warehouse, and turning the VIEW cannot fix that
        # because the view turns map and floor together. --map-pose places the grid
        # instead: it is the pose of the map's origin expressed in world terms.
        #
        # to_screen is (x,y) -> (W/2 + (x-cx)*ppm, H/2 - (y-cy)*ppm), so a world
        # translation becomes (tx*ppm, -ty*ppm) on screen and a world rotation
        # becomes the NEGATIVE angle (screen y points down). Qt applies painter
        # transforms innermost-last, so translate is written first to be applied
        # second: rotate about the world origin, then translate.
        placed = self.map_pose is not None
        if placed:
            mx, my, myaw = self.map_pose
            p.save()
            p.translate(mx * self.view.ppm, -my * self.view.ppm)
            sx, sy = self.view.to_screen(0.0, 0.0)
            p.translate(sx, sy)
            p.rotate(-math.degrees(myaw))
            p.translate(-sx, -sy)
    ...
    def _draw_panel(self, p):
        with self.state.lock:
            ...
            loc, loc_ok_t, sim_t = st.loc, st.loc_ok_t, st.sim_t
        ...
        pad, lh, w = 10, 15, 330
        n = (len(rows) + 10 + len(warns) + (1 if (n_route or route_msg) else 0)
        ...
        if gt:
            ...
            p.drawText(16, y, 'pose  no ground truth (localization only)'
                       if not self.args.gt_topic else
                       'pose  waiting for %s' % self.args.gt_topic)
            y += lh
        y += lh
        if loc is not None:
            lx, ly, lyaw, cov, _ = loc
            if cov >= LOC_NOT_LOCALIZED:
                # Say how stale the last real fix is: "searching" reads very
                # differently at 2 s than at 60 s.
                age = ('' if (loc_ok_t is None or sim_t is None)
                       else '  last fix %.0fs ago' % (sim_t - loc_ok_t))
                p.setPen(QPen(C_BAD))
                p.drawText(16, y, 'loc   SEARCHING (cov 9999)%s' % age)
            else:
                p.setPen(QPen(C_OK))
                p.drawText(16, y, 'loc   x %+.2f y %+.2f yaw %+.1f  sd %.2f m'
                           % (lx, ly, math.degrees(lyaw), math.sqrt(cov)))
            p.setPen(QPen(QColor(210, 215, 225)))
            y += lh
        for key, label, err, mode, shown, aligned, topic, has_data in est_rows:
            ...
            if topic == 'auto':
                ...
                txt = '%-5s raw (odom frame)' % label
            elif aligned:
                txt = '%-5s aligned, no GT to compare' % label
            elif not self.args.gt_topic:
                txt = '%-5s no GT to align to; use --align none' % label
            else:
...
def parse_args(argv):
    ...
    ap.add_argument('--gt-topic', default='/ground_truth/odometry',
                    help="'' disables ground truth entirely: no trail, no error "
                         "column, and the front-end odometry becomes the clock. "
                         "This is the localization-mode setting.")
    ...
    ap.add_argument('--map-pose', dest='map_pose', default='',
                    metavar='X,Y,YAW_DEG',
                    help='Place the map overlay in world coordinates. A SLAM '
                         'database is built in the FRONT-END\'s frame (anchored '
                         'where it initialised), not the Gazebo world frame, so it '
                         'draws offset and rotated. This is the pose of the map '
                         "origin in world terms, e.g. '1.8,9.0,-90' for a run that "
                         'started at the spawn. Empty = draw the map as-is.')
    ap.add_argument('--loc-topic', default='',
                    help="rtabmap PoseWithCovarianceStamped, e.g. "
                         "/localization_pose. OFF by default: RViz is the place "
                         "to watch localization, and an always-on row would sit "
                         "at 'no data' during ordinary sim runs. A 9999 "
                         "covariance diagonal means NOT relocalized.")
