...
"""Bake an exact nav2 occupancy map from the warehouse world's COLLISION meshes.
    ...
    python3 ~/wil_project/bake_map.py --preview     # look before you leap
    python3 ~/wil_project/bake_map.py               # writes maps/baked/
        ...
        for col in link.findall('collision'):
            ...
            if mesh is not None:
                ...
                if uri.startswith('model://'):
                    ...
                    path = os.path.join(models_dir, model, uri)
...
def main():
    ...
    pkg = os.path.join(here, 'aws-robomaker-small-warehouse-world')

    ap = argparse.ArgumentParser(
        description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument('--world', default=os.path.join(
        pkg, 'worlds', 'small_warehouse', 'small_warehouse.world'))
    ap.add_argument('--models', default=os.path.join(pkg, 'models'))
