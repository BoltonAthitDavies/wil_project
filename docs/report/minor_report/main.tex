...
% Queried material. Red marks a result, number or condition that is measured but
% NOT yet trustworthy as evidence -- an unexplained mechanism, a confound, an
...
% is correctly measured and understood is a finding, not a query. Everything in
% red needs an action before it can appear in the major report.
\definecolor{flagred}{HTML}{B00000}
\newcommand{\flag}[1]{\textcolor{flagred}{#1}}
% Whole-paragraph query. Sets the paragraph in red behind a bracketed label so it
% survives greyscale printing, where colour alone would vanish.
...
\input{figures/pipeline_performance}
