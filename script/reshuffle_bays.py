...
"""Bay groups that shuttle between bay centres, driven by TrajectoryFollower.
    ...
    python3 script/reshuffle_bays.py [--seed N] [--speed V] [--per-bay-max N] [--dry-run]
...
def kinematic_block(speed, dwell, pts, phase=0.0, dwell_far=None, dwell_home=None):
    """Static prop + kinematic waypoint follower. Any number of waypoints."""
    # 6 decimals, not 4: each waypoint is rounded independently, so at %.4f the
    # DIFFERENCE between two of them can vary by 1e-4 across a group -- a 0.1 mm
    # formation shear that shows up in any exact rigid-translation check.
    wps = ''.join('          <waypoint>%.6f %.6f</waypoint>\n' % (x, y) for x, y in pts)
    return ('      <static>true</static>\n'
            ...
            '        <loop>true</loop>\n'
            '        <dwell>%.2f</dwell>\n'
            '        <dwell_far>%.3f</dwell_far>\n'
            '        <dwell_home>%.3f</dwell_home>\n'
            '        <phase>%.3f</phase>\n'
            '        <waypoints>\n%s'
            '        </waypoints>\n'
            '      </plugin>\n') % (speed, dwell,
                                  dwell if dwell_far is None else dwell_far,
                                  dwell if dwell_home is None else dwell_home,
                                  phase, wps)
...
def main():
    ...
    out_world = sys.argv[sys.argv.index('--out') + 1] if '--out' in sys.argv else WORLD
    # --pack stock=north,west=south,... : instead of picking home bays at random
    # inside a zone, take them from one END of it. Groups then start at a wall and
    # the furthest in-zone destination is the far end, giving a full-length sweep
    # rather than a short hop from somewhere in the middle.
    pack_spec = {}
    if '--pack' in sys.argv:
        for part in sys.argv[sys.argv.index('--pack') + 1].split(','):
            z, side = part.split('=')
            pack_spec[z.strip()] = side.strip()
    # --occ stock=2,east_s=1,... : how many bays in each zone host a group.
    occ_spec = {}
    if '--occ' in sys.argv:
        for part in sys.argv[sys.argv.index('--occ') + 1].split(','):
            z, n = part.split('=')
            occ_spec[z.strip()] = int(n)
    dry = '--dry-run' in sys.argv
    rng = random.Random(seed)

    text = open(out_world if os.path.isfile(out_world) else WORLD).read()
    ...
    if occ_spec:
        # Per-zone occupancy: pick exactly N home bays inside each named zone.
        byzone = defaultdict(list)
        for i in range(len(BAYS)):
            byzone[ZONE[i]].append(i)
        home = []
        for z, n in occ_spec.items():
            if z not in byzone:
                raise SystemExit('unknown zone %r (have %s)' % (z, sorted(byzone)))
            if n > len(byzone[z]):
                raise SystemExit('zone %s has %d bays, asked for %d'
                                 % (z, len(byzone[z]), n))
            side = pack_spec.get(z)
            if side:
                ordered = sorted(byzone[z], key=lambda i: (BAYS[i][3] + BAYS[i][4]) / 2,
                                 reverse=(side == 'north'))
                home += ordered[:n]
            else:
                home += rng.sample(byzone[z], n)
        home = sorted(home)
    else:
    ...
    print('\nWrote %s' % os.path.basename(out_world))

    # --- mirror the same layout into the static world, frozen -------------------
    # Only when writing the canonical dynamic world. A --out variant is a scenario
    # of its own; mirroring it would clobber the static twin of the main world.
    if os.path.abspath(out_world) != os.path.abspath(WORLD):
        return
