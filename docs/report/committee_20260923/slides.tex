...
\begin{frame}{What exists at the cutoff}
  \centering
  \resizebox{0.80\textwidth}{!}{\input{../minor_report/figures/system_architecture}}
...
\begin{frame}{Finding 1: the dead-reckoning floor is higher than expected}
  ...
  \begin{alertblock}{}
    ...
    \SIrange{0.10}{0.88}{\percent} drift. \textbf{The project finally has a
    proprioceptive baseline.}
  \end{alertblock}
\end{frame}

% The control belongs on its own slide, immediately after. Presenting the table
% above and then withdrawing it is deliberate: the committee sees the result the
% way the project saw it, then sees what the control did to it.
\begin{frame}{\ldots{}but the two families were never tuned alike}
  \begin{itemize}
    \item The EKF's noise model is \emph{measured from each recording}.
    \item VINS and ORB read \emph{four constants from a hand-set file}.
    \item Tuning was an uncontrolled variable, confounded with the estimator.
  \end{itemize}

  \vspace{0.4em}
  Give the EKF that same file and re-run. Aligned ATE RMSE, metres:
  ...
  \begin{tabular}{lrrrrr}
    \toprule
     & 000 & 001 & 002 & 003 & 004 \\
    \midrule
    EKF, as tuned & 0.19 & 0.31 & 0.26 & 0.42 & 0.49 \\
    EKF, VSLAM config & \bad{1.31} & \bad{0.68} & \bad{0.87} & \bad{1.23} & \bad{1.58} \\
    VINS-Fusion & 0.62 & 0.38 & 0.56 & 0.78 & 0.43 \\
    \bottomrule
  \end{tabular}

  \vspace{0.5em}
  \begin{alertblock}{}
    \textbf{VINS wins on five of five.} The ranking is a tuning result, not an
    architecture result.
  \end{alertblock}
\end{frame}

\begin{frame}{Why that finding matters}
  It is the missing baseline --- plus an honest account of what it is worth.

  \vspace{0.8em}
  \begin{itemize}
    \item Before this, every trajectory figure reported visual error
      \emph{with nothing to compare it to}.
    \item Four hypotheses were registered before the metrics were computed; one
      was falsified by its own data. A fifth, added after, asked whether the
      EKF's lead survived matched tuning. It did not.
    \item An ablation localises the whole effect to one term: the gyroscope
      process noise.
  \end{itemize}

  \vspace{0.8em}
  The honest reading: \textbf{the noise model is worth more than the
  architecture over this range.} Vision must still justify itself where dead
  reckoning fails.
\end{frame}
...
\begin{frame}{Where I need the committee's guidance}
  \begin{enumerate}
    \item \textbf{Scope.} The static-scene comparison reduces to a tuning
      difference, and neither visual pipeline has ever been tuned to this
      platform. Equalise that first --- tune both and re-run --- or settle the
      question where dead reckoning is expected to fail regardless?
